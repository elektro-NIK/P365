...
from .views import TrackView, GetTrackView, TrackEditView, TrackChangeStatusView, TrackDeleteView, GetTracksTableView


urlpatterns = [
    url(r'^(\d+)/$', TrackView.as_view(),                           name='track'),
    url(r'^(\d+)/geojson/$', GetTrackView.as_view(),                name='geojson_get_track'),
    url(r'^(\d+)/change_status/$',
